...
var stories = [
	myWay = [
		...
		"<strong>Information</strong> The lyrics seemed simple and not paranormal. However, the first stanza makes me feel horrified. Well, it says: And now, the end is here",
...
function next() {
	console.log(page);
	if (page < myWay.length - 1) {
		document.getElementById("story").innerHTML = "<p>" + stories[story][++page]  + "</p><button onclick=next()</button> <button onclick=previous()>Previous</button>";
	}
	if (page == myWay.length - 1) {
...
function end() {
	document.getElementById("story").innerHTML = "<button onclick=pickStory(0)>My Way's Curse</button> <button onclick=pickStory(1)>The Curse of One's Death</button>";
